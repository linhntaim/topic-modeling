...
def update_graph(n_clicks, stored_data, stored_metadata, stored_umap):
    ...
    embedding = umap_model.fit_transform(
        calculate_document_topic_term_matrix(
            document_topic_matrix,
            topic_term_matrix,
        )
    )
...
def calculate_document_topic_term_matrix(document_topic_matrix, topic_term_matrix):
    document_topic_term_matrix = np.array(
        [
            [np.multiply(topic_term_matrix[topic_id], topic_probability)
             for topic_id, topic_probability in enumerate(topics)]
            for topics in document_topic_matrix
        ]
    )
    n_samples = document_topic_term_matrix.shape[0]
    return document_topic_term_matrix.reshape(n_samples, -1)
