...
class SpecialCharsExtractor(DocsExtractor):
    def from_db(self, docs_db: DocsDB, dir_path: str, raw_dir_path: str = None):
        ...
        special_chars = sorted(
            list(set(list(chain.from_iterable([list(self._remove_known_chars(doc['text'])) for doc in docs])))),
            reverse=True,
        )
    ...
    def _remove_known_chars(self, text):
        regexes = [
            ...
            re.compile(r'[¥£₦￥Ȼ¢₹ȼ€]+'),
            re.compile(
                r'[𝑎𝑐𝑑𝑒𝑖𝑗𝑘𝑙𝑚𝑛𝑝𝑞𝑟𝑠𝑡𝑢𝑣𝑤𝑥𝑦𝑧𝐴𝐵𝐶𝐷𝐸𝐹𝐺𝐻𝐼𝐾𝑀𝑁𝑂𝑃𝑄𝑅𝑆𝑇𝑈𝑉𝑊𝑋𝑌𝑍𝛼𝜌𝜏𝜂𝛽ℓαπθβϕσφγελχρɛΔ∆ξƩηωτµƐμδ∑ΖŸ훿휀훼휋훽휎]+'),
            re.compile(r'[⊆∗⊉∈≠≈±∼∪∕→²₂∣≥⫆⊗∃×√§⁓≤³‰∞⋅÷∀]+'),
            # CJK
            ...
            # Vietnam
            re.compile(
                r'[ĂăÂâĐđÊêÔôƠơƯưÁáÀàÃãẢảẠạẤấẦầẪẫẨẩẬậẮắẰằẴẵẲẳẶặÉéÈèẼẽẺẻẸẹẾếỀềỄễỂểỆệÍíÌìĨĩỈỉỊịÓóÒòÕõỎỏỌọỐốỒồỖỗỔổỘộỚớỜờỠỡỞởỢợÚúÙùŨũỦủỤụỨứỪừỮữỬửỰựÝýỲỳỸỹỶỷỴỵ]+'),
        ]
