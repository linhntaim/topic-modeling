...
# @see https://github.com/piskvorky/gensim/blob/release-3.8.3/gensim/models/wrappers/ldamallet.py
class LdaMallet(LdaBase, utils.SaveLoad, BaseTopicModel):
    """Python wrapper for LDA using `MALLET <https://mallet.cs.umass.edu/>`_.
    ...
    Warnings
    --------
    This is **only** python wrapper for `MALLET LDA <https://mallet.cs.umass.edu/>`_,
    ...
    def __init__(self,
        ...
        docs_topic_threshold : float, optional
        ...
        self.alpha = alpha
        self.beta = beta
        self.docs_topic_threshold = docs_topic_threshold
    ...
    def convert_input(self, corpus, infer=False, serialize_corpus=True):
        ...
        cmd = \
            self.mallet_path + \
            ' import-file --preserve-case TRUE --keep-sequence TRUE ' \
            '--remove-stopwords FALSE --token-regex "\\S+" --input %s --output %s'
        if infer:
    ...
    def train(self, corpus):
        ...
        cmd = self.mallet_path + ' train-topics --input %s --num-topics %s --num-threads %s --num-iterations %s --random-seed %s ' \
                                 '--optimize-interval %s --optimize-burn-in %s --alpha %s --beta %s ' \
                                 '--output-state %s --inferencer-filename %s --output-topic-keys %s --output-doc-topics %s --doc-topics-threshold %s '

        cmd = cmd % (
            self.fcorpusmallet(), self.num_topics, self.num_threads, self.num_iterations, self.random_seed,
            self.optimize_interval, self.optimize_burn_in, self.alpha, self.beta,
            self.fstate(), self.finferencer(), self.ftopickeys(), self.fdoctopics(), self.docs_topic_threshold,
        )
    ...
    def __getitem__(self, bow, iterations=100):
        ...
        cmd = cmd % (
            ...
            self.fdoctopics() + '.infer', iterations, self.docs_topic_threshold, str(self.random_seed)
        )
    ...
    def load_word_topics(self):
        ...
        if hasattr(self.id2word, 'token2id'):
            ...
            word2id = utils.revdict(self.id2word)
    ...
    def get_version(self, direc_path):
        ...
        try:
            ...
            try:
                doc = XmlElementTree.parse(xml_path + "pom.xml").getroot()
    ...
    def get_topic_term_matrix(self):
        return [[number.probability(probability) for probability in terms] for terms in self.get_topics()]

    def get_document_topic_matrix(self, dtm_runner: DtmRunner):
        document_topic_matrix = []
        for document_topics in self.load_document_topics():
            topics = [0] * self.num_topics
            for topic_id, topic_probability in document_topics:
                topics[topic_id] = number.probability(topic_probability)
            document_topic_matrix.append(topics)
    ...
    def _extract_visualizing_data(self, corpus, dictionary):
        ...
        beta = 0.01
        fnames_argsort = numpy.asarray(list(dictionary.token2id.values()), dtype=numpy.int_)
        ...
        doc_topic_dists = numpy.array(
            [[topic_probability for topic_id, topic_probability in document_topics] for document_topics in
             self.load_document_topics()]
        )

        topics = self.get_topics()
        topic_term_dists = topics[:, fnames_argsort]
